...
<template>
  ...
  <Transition name="fade-up">
    <div 
      v-if="toolbarState.show"
      ref="toolbarRef"
      class="image-toolbar"
      :style="{ top: toolbarState.top + 'px', left: toolbarState.left + 'px' }"
    >
      <button class="tool-btn" @click.stop="alignImage('left')" title="左对齐">
        <svg viewBox="0 0 24 24" width="16" height="16"><path fill="currentColor" d="M3 19h18v-2H3v2zm0-4h12v-2H3v2zm0-4h18V9H3v2zm0-6v2h12V3H3z"/></svg>
      </button>
      <button class="tool-btn" @click.stop="alignImage('center')" title="居中">
        <svg viewBox="0 0 24 24" width="16" height="16"><path fill="currentColor" d="M3 19h18v-2H3v2zm4-4h10v-2H7v2zm-4-4h18V9H3v2zm4-6v2h10V3H7z"/></svg>
      </button>
      <button class="tool-btn" @click.stop="alignImage('right')" title="右对齐">
        <svg viewBox="0 0 24 24" width="16" height="16"><path fill="currentColor" d="M3 19h18v-2H3v2zm6-4h12v-2H9v2zm-6-4h18V9H3v2zm6-6v2h12V3H9z"/></svg>
      </button>
      <div class="divider"></div>
      <button class="tool-btn delete-btn" @click.stop="removeImage" title="删除图片">
        <svg viewBox="0 0 24 24" width="16" height="16"><path fill="currentColor" d="M19 4h-3.5l-1-1h-5l-1 1H5v2h14V4zM6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12z"/></svg>
      </button>
    </div>
  </Transition>
</Teleport>
...
/* --- 布局修复：核心点 --- */
.edit-view { 
  position: fixed; /* 覆盖全屏，防止内容撑破外部容器 */
  top: 0; left: 0; right: 0; bottom: 0;
  z-index: 100;
  background: #f8fafc; 
  padding: 20px;
  overflow-y: auto; /* 确保这里有滚动条 */
  display: flex;
...
.edit-container { 
  ...
  height: fit-content; /* 根据内容自适应，但受父级滚动控制 */
}
...
.rich-editor :deep(.resizable-img-container::-webkit-resizer) {
  ...
  background-size: 4px 4px; /* 波点纹理感 */
}

/* --- 按钮与表单 --- */
.form-body { display: flex; flex-direction: column; gap: 18px; }
.input-group { display: flex; flex-direction: column; gap: 8px; }
.input-group label { font-weight: 700; color: #64748b; font-size: 13px; }
...
.image-toolbar {
  position: absolute;
  z-index: 10000;
  background: rgba(15, 23, 42, 0.85); /* 深色磨砂玻璃 */
