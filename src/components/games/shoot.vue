...
const progressPercent = computed(() => {
  ...
  // 计算已完成的比例
  return (currentIndex.value / totalWordsCount.value) * 100;
});

const remainingCount = computed(() => {
  return totalWordsCount.value - currentIndex.value;
});
...
<template>
  <div class="war-viewport" :class="{ 'shake': isShaking, 'damage-fx': isDamageActive, 'flash-white': isFlashing }">
    ...
    <div v-for="b in bulletHoles" :key="b.id" class="hole-fx"
      :style="{ left: b.x + 'px', top: b.y + 'px', transform: `rotate(${b.rotate}deg)` }"></div>
    ...
    <div class="hud-bar">
      ...
      <div class="hud-section progress-main">
        <div class="tag">MISSION_PROGRESS ({{ currentIndex }}/{{ totalWordsCount }})</div>
        <div class="global-progress-container">
          <div class="global-progress-bar" :style="{ width: progressPercent + '%' }">
        ...
        <div class="remaining-tag">REMAINING: {{ remainingCount }} UNITS</div>
      </div>

      <div class="hud-section center">
        <div class="tag">TIME_WINDOW</div>
        <div class="hp-track">
          <div class="hp-fill" :style="{ width: timeLeft + '%' }"></div>
        </div>
      </div>
    ...
    <div class="battle-main">
      <div class="word-monitor">
        ...
        <div class="corner-frame t-l"></div>
        <div class="corner-frame b-r"></div>
      ...
      <div class="options-container">
        <div v-for="opt in options" :key="opt" class="war-btn" :class="{
          'hit': currentBtnState?.answer === opt && currentBtnState?.correct,
...
.word-monitor {
  ...
  box-shadow: inset 0 0 30px rgba(255, 157, 0, 0.2);
  overflow: hidden;
  isolation: isolate;
}
...
.miss {
  background: rgba(255, 60, 0, 0.2) !important;
...
.spark {
  ...
  border-radius: 50%;
  opacity: 0;
  animation: sparkScatter 0.1s ease-out forwards;
}
...
.muzzle-glow-soft {
  ...
  transform: translate(-50%, -50%);
  opacity: 0.6;
  animation: glowFade 0.1s forwards;
}
...
.damage-fx {
  box-shadow: inset 0 0 100px rgba(255, 60, 0, 0.5);
}
...
/* 入场动画保持 zoom */
.zoom-enter-active {
  transition: all 0.4s cubic-bezier(0.15, 0.85, 0.35, 1.2);
}
