  ...
  <div class="hacker-viewport" :class="{ 'high-combo-glow': combo >= 5 }">
    ...
    <div v-if="words.length > 0" id="terminal" :class="{ 'error-shake': isShaking }">
      ...
      <div id="timer-wrapper">
        <div id="timer-bar" :style="{ width: timeLeft + '%' }"></div>
      </div>
      <div id="progress-wrapper">
        <div id="progress-fill" :style="{ width: (currentIndex / words.length) * 100 + '%' }"></div>
      </div>
      ...
      <div id="options-grid">
        <button v-for="opt in options" :key="opt" class="btn-opt" :disabled="currentBtnState !== null"
    ...
    <Teleport to="body">
      <div v-for="p in floatPoints" :key="p.id" class="float-pts-container"
      ...
      <div class="result-card">
        ...
        <div class="final-stats">
          ...
          <div class="stat-item">
            ...
            <span class="stat-value">{{ words.length ? Math.round(((words.length - errorLog.length) / words.length) *
              100) : 0 }}%</span>
          </div>
        </div>
...
@keyframes pts-spring-blast {
  0% {
    transform: translate(-50%, 0) scale(0) rotate(-30deg);
    opacity: 0;
  }

  25% {
    transform: translate(-50%, -100%) scale(1.7) rotate(15deg);
    opacity: 1;
  }

  100% {
    transform: translate(-50%, -280%) scale(0.8);
    opacity: 0;
  }
}

@keyframes pts-shine-move {
  100% {
    left: 150%;
  }
}
...
.high-combo-glow {
  animation: bg-pulse-intense 2s infinite;
}

@keyframes bg-pulse-intense {

  0%,
  100% {
    box-shadow: inset 0 0 100px rgba(112, 0, 255, 0.1);
  }

  50% {
    box-shadow: inset 0 0 150px rgba(112, 0, 255, 0.25);
  }
}
...
/* --- 空状态 --- */
.empty-system {
  border: 1px dashed var(--neon-red) !important;
...
/* --- 滚动条整体优化 --- */
.error-scroll-area::-webkit-scrollbar {
  width: 5px;
  /* 窄一点更精致 */
}

/* 滚动条轨道 */
.error-scroll-area::-webkit-scrollbar-track {
  background: rgba(255, 255, 255, 0.05);
  border-radius: 10px;
}

/* 滚动条滑块 (Thumb) */
.error-scroll-area::-webkit-scrollbar-thumb {
  background: var(--neon-red);
  /* 使用你的霓虹红变量 */
  border-radius: 10px;
...
/* 滑块悬停效果 */
.error-scroll-area::-webkit-scrollbar-thumb:hover {
  background: #ff4d88;
  /* 稍微亮一点的红色 */
}
